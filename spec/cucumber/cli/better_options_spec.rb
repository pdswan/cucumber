require 'spec_helper'
require 'yaml'
require 'cucumber/cli/better_options'

module Cucumber
  module Cli
    describe BetterOptions do
      ...
      describe ".from_args" do
        def when_parsing(args)
          yield
          @options = BetterOptions.from_args(prepare_args(args), output_stream)
        end
        ...
        context '-l LINES or --lines LINES' do
          it "adds line numbers to args" do
            after_parsing('-l24 FILE') { expect(options[:paths]).to eq ['FILE:24'] }
          end
        end
        ...
        context '-b or --backtrace' do
          it "turns on cucumber's full backtrace" do
            after_parsing("-b") do
              expect(options[:use_full_backtrace]).to be true
            end
          end
        end
        ...
        describe "dry-run" do
          ...
          it "sets snippets to false when no-snippets provided after dry-run" do
            ...
            after_parsing('--dry-run --no-snippets') do
              expect(options[:snippets]).to be false
            end
          end
          ...
          it "sets snippets to false when no-snippets provided before dry-run" do
            ...
            after_parsing('--no-snippets --dry-run ') do
              expect(options[:snippets]).to be false
            end
          end
        end
